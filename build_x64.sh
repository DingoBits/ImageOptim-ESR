#!/bin/bash
#
# x86_64 Build Script for ImageOptim-ESR components
...
cp -r dependencies x64_pkgs

WORKSPACE="$(pwd)/x64_workspace"
...
export CPPFLAGS="-Ofast -flto -I$WORKSPACE/include"
...
if [ -f "$WORKSPACE/lib/libz.a" ]; then
	...
	echo "Building zlib"
	cd zlib || exit
	run arch -x86_64 ./configure --prefix="$WORKSPACE" --static --64 --archs="-arch x86_64"
...
if [ -f "$WORKSPACE/lib/libpng.a" ]; then
	...
	cd libpng || exit
	run cmake . -DCMAKE_OSX_ARCHITECTURES="$ARCH" -DCMAKE_BUILD_TYPE=RELEASE -DPNG_ARM_NEON=off -DPNG_INTEL_SSE=on -DPNG_SHARED=OFF -DPNG_STATIC=ON -DPNG_TESTS=OFF -DZLIB_INCLUDE_DIR="${WORKSPACE}/include" -DZLIB_LIBRARY_RELEASE="${WORKSPACE}/lib/libz.a"
	run make -j "$NPROC"
...
if [ -f "$WORKSPACE/lib/libjpeg.a" ]; then
	...
	cd mozjpeg || exit
	run cmake . -DCMAKE_OSX_ARCHITECTURES="$ARCH" -DCMAKE_BUILD_TYPE=RELEASE -DENABLE_SHARED=OFF -DENABLE_STATIC=ON -DPNG_SUPPORTED=OFF -DWITH_12BIT=ON -DWITH_ARITH_DEC=ON -DWITH_ARITH_ENC=ON -DWITH_MEM_SRCDST=ON -DWITH_SIMD=ON -DCMAKE_INSTALL_PREFIX="$WORKSPACE"
	run make -j "$NPROC"
	run make install/strip
	cd .. || exit
fi
...
if [ -f "$WORKSPACE/bin/pngquant" ]; then
	...
	if [ -d "/usr/local/opt/libomp" ] ; then
		...
		cp -r "/usr/local/opt/libomp/include/." "$WORKSPACE/include"
		cp "/usr/local/opt/libomp/lib/libomp.a" "$WORKSPACE/lib/libomp.a"
	fi
	cd pngquant || exit
...
cd zopfli || exit
	...
	if [ ! -f Makefile.backup ]; then
	    sed -i.backup 's/\-O2/\-Ofast/' Makefile
	fi
...
if [ -f "$WORKSPACE/lib/libdeflate.a" ]; then
	...
	cd libdeflate || exit
	run arch -x86_64 make install -j "$NPROC" PREFIX="$WORKSPACE"
